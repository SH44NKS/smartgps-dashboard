...
function sgMigrateLegacy_(source, db) {
  ...
  var map = {
    ...
    'Cancelamento': function (r) { return sgAppendWithdrawal_(db, { date:r[0], client:r[1], doc:r[2], plate:r[3], tracker:r[4], obs:r[5] || r[8], technician:r[6], status:r[7] || 'Contato pendente', origem:'Migracao' }); },
...
function sgSummary_(records) {
  ...
  records.forEach(function (record) {
    ...
    var data = record.values.Data || record.values['Data Entrada'] || record.values['Data Servico'] || record.values['Criado em'] || '';
